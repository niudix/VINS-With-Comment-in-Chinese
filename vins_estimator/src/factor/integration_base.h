...
{
  public:
  // 初始化IntegrationBase类，给 _acc_0 _gyr_0 _linearized_ba _linearized_bg noise 赋值;
    ...
    IntegrationBase(const Eigen::Vector3d &_acc_0, const Eigen::Vector3d &_gyr_0,
        ...
        : acc_0{_acc_0}, gyr_0{_gyr_0}, linearized_acc{_acc_0}, linearized_gyr{_gyr_0},
          ...
          //初始化的这些矩阵中,covariance是协方差矩阵，delta_p（alpha，位移的变化量)

    {
        //噪声协方差矩阵(k时刻acc,gyr的噪声，k+1时刻acc,gyr的噪声。陀螺仪和加速度计的随即游走噪声)
    ...
    // 把acc 和 dt, gyr 插入buffer （每一个IMU数据都存储进去）
    ...
    //其中, delta_q 旋转矩阵（第i帧imu数据相对第k帧图像的旋转矩阵)
    // ba 加速度计零偏
                            ...
                            Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
        ...
        // delta_q 四元数
        Vector3d un_acc_0 = delta_q * (_acc_0 - linearized_ba); //把上一时刻加速度的值转化到图像帧的坐标系
        Vector3d un_gyr = 0.5 * (_gyr_0 + _gyr_1) - linearized_bg; //陀螺仪作中值积分（用两帧之间陀螺仪的均值来替代陀螺仪这段时间的值）
        result_delta_q = delta_q * Quaterniond(1, un_gyr(0) * _dt / 2, un_gyr(1) * _dt / 2, un_gyr(2) * _dt / 2); //推导见VINS_MONO笔记 从第i帧的四元数推导第i+1帧的四元数
        Vector3d un_acc_1 = result_delta_q * (_acc_1 - linearized_ba); //通过i+1时刻的四元数，求出i+1时刻的加速度计在k帧图像的表示
        Vector3d un_acc = 0.5 * (un_acc_0 + un_acc_1); //用平均值代替这段时间的变化量 (平均值*delta(t)=积分)
        result_delta_p = delta_p + delta_v * _dt + 0.5 * un_acc * _dt * _dt; //位置变化量 p=p0+vo*t+0.5*a*t^2
        result_delta_v = delta_v + un_acc * _dt;   //速度变化量
        result_linearized_ba = linearized_ba;   //随即游走保持不变
        result_linearized_bg = linearized_bg;         

        //这个update_jac 恒为1，所以一直update
        if(update_jacobian)
        {
    ...
    void propagate(double _dt, const Eigen::Vector3d &_acc_1, const Eigen::Vector3d &_gyr_1)
    {
        ...
        // acc_0 gyr_0  都代表k时刻加速度计的值
        // acc_1 gyr_1  代表k+1时刻加速度计和陀螺仪的值
        // delta_p, delta_q, delta_v ： k时刻的 alpha beta gamma

        //入参：上一时刻和这一时刻加速度，角速度计的值
        //出参：linearized_ba, linearized_bg, result_delta_p, result_delta_q, result_delta_v, result_linearized_ba, result_linearized_bg,
        ...
        //                    linearized_ba, linearized_bg);
        delta_p = result_delta_p; //更新后的位置
        delta_q = result_delta_q; //更新后的速度
        delta_v = result_delta_v; //更新后的角度
        linearized_ba = result_linearized_ba; //加速度的bias
        linearized_bg = result_linearized_bg; //gyro 的 bias
        delta_q.normalize();
        sum_dt += dt;
        acc_0 = acc_1;
        gyr_0 = gyr_1;  
